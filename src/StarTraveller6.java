...
public class StarTraveller6 {
	...
	int[] search(int[] ships, int[] stars) {
		...
		while (endTime > System.currentTimeMillis()) {
			int nv = Integer.MAX_VALUE, nex_[] = Arrays.copyOf(nex, S), rev_[] = Arrays.copyOf(rev, S);
			for (int i = ships.length; i < S; ++i) {
				...
				for (int j = i; j != -1; j = nex[j]) {
					...
					for (int k = 0; k < S; ++k) {
						...
						if (nv > tv) {
							nv = tv;
							System.arraycopy(nex, 0, nex_, 0, S);
							System.arraycopy(rev, 0, rev_, 0, S);
							if (nex_[j] == -1) {
								nex_[rev_[i]] = -1;
							} else {
								nex_[rev_[i]] = nex_[j];
				...
				for (int j = ships.length; j < S; ++j) {
					...
					if (a == b) {
						...
						if (nv > tv) {
							...
							System.arraycopy(rev, 0, rev_, 0, S);
							if (as > bs) {
								reverse(rev_[i], j, rev_, nex_);
								nex_[b] = a;
								nex_[j] = i;
								rev_[a] = b;
								rev_[i] = j;
							} else {
								reverse(rev_[j], i, rev_, nex_);
								nex_[a] = b;
								nex_[i] = j;
								rev_[b] = a;
								rev_[j] = i;
