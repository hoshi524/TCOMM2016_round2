...
public class StarTravellerAfter {
    ...
    public int[] makeMoves(int[] ufos, int[] ships) {
        ...
        if (t == 1) {
            ...
            int[] res = Arrays.copyOf(ships, ships.length);
            {
                ...
                boolean[] us = new boolean[p], uu = new boolean[f];
                for (int i = 0; i < p; ++i) {
                    if (us[i]) continue;
                    for (int j = 0; j < f; ++j) {
                        if (uu[j]) continue;
                ...
                while (true) {
                    int d = Integer.MAX_VALUE, ship = 0, ufo = 0;
                    for (int i = 0; i < p; ++i) {
                        if (us[i]) continue;
                        for (int j = 0; j < f; ++j) {
                            if (uu[j]) continue;
                            if (d > dist[ships[i]][ufos[j * 3 + 1]]) {
                                d = dist[ships[i]][ufos[j * 3 + 1]];
                                ship = i;
                                ufo = j;
                            }
                        }
