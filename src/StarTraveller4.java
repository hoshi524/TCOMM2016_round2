...
public class StarTraveller4 {
	...
	public int init(int[] stars) {
		...
		for (int i = 0; i < s; ++i) {
			for (int j = 0; j < s; ++j) {
				...
				dist[i][j] = (int) (Math.sqrt(x * x + y * y) * 10000);
			}
		}
		search = null;
		return 0;
	...
	int[] search(int[] ships, int[] stars) {
		...
		int buf[] = new int[S], bi = 0, best[] = Arrays.copyOf(rev, S), bv = v;
		long remainTime = endTime - System.currentTimeMillis();
		long startTemp = 5000;
		long temp = startTemp * remainTime / MAX_TIME;
		while (remainTime > 0) {
			for (int roop = 0; roop < 0xffff; ++roop) {
				move: {
					...
					if (v > tv || Math.exp((double) (v - tv) / temp) * 0xfffff > x.next(0xfffff)) {
						...
						if (bv > v) {
							bv = v;
							System.arraycopy(rev, 0, best, 0, S);
						}
					}
				}
				move: {
			...
			temp = startTemp * remainTime / MAX_TIME;
		}
		if (false) {
			int tv = 0;
			for (int i = 0; i < S; ++i) {
			...
			if (bv != tv) throw new RuntimeException();
		}
		int[] res = new int[this.s];
		Arrays.fill(res, -1);
		for (int i = 0; i < S; ++i) {
			if (rev[i] != -1) res[trans[i]] = trans[best[i]];
		}
		return res;
	}
